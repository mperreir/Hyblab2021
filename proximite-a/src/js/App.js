...
class App extends  React.Component {
    ...
    createSites = async function() {
        let stringAdresse = this.state.adresse.rue.replace(' ', '+') + '+' + this.state.adresse.codepostal + '+' + this.state.adresse.ville.replace(' ', '+')
        ...
        if (lieux.lieux.length >= 2 && typeof lieux.lieux !== "undefined"){let adresse2 = await (await fetch(`http://localhost:8080/proximite-a/api/coordinates/${lieux.lieux[1].lat}_${lieux.lieux[1].lon}`)).json();
            let name2 = ""
        ...
        if (lieux.lieux.length >= 3 && typeof lieux.lieux !== "undefined") {let adresse3 = await (await fetch(`http://localhost:8080/proximite-a/api/coordinates/${lieux.lieux[2].lat}_${lieux.lieux[2].lon}`)).json();
            let name3 = ""
            if (typeof lieux.lieux[0].tags.name !== "undefined") {
        ...
        if (typeof lieux.surprise !== "undefined"){let adresseSurp = await (await fetch(`http://localhost:8080/proximite-a/api/coordinates/${lieux.surprise.lat}_${lieux.surprise.lon}`)).json();
            console.log(adresseSurp)
            let nameSurp = ""
            if (lieux.surprise.tags.name) {
                nameSurp = lieux.surprise.tags.nameimport React from 'react';
                import Home from './Home';
                import Theme from './Theme';
                import Attributs from './Attributs';
                import Moyen from './Moyen';
