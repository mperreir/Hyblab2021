...
class Choice {
    async addPOI(origin, arrivee, transport, list_POI) {
        const list = await pointInteret(origin, arrivee, this.theme, transport)
        if (list) {
            const indexResult2 = getShortestExcursion(list);
            const data = list[indexResult2]
            const POI = extractUtilsValue(data)
